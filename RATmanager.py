...
class TimerLogic:
    ...
    def __init__(self, name, duration, component):
        ...
        # Her legges inputet fra "Enter RAT number i UI"
        #self.RATnumber = values['-RATnumber-']
        self.name = name # = "RAT" + " " + RATnumber
        # Her legges inputet fra "Enter number of minutes i UI"
        self.duration = duration # = int(values['-MINUTES-'])
        self.component = component

    def started(self):
        self.stm.start_timer('t', self.duration * 1000)
        self._logger.debug('RAT {} with duration {} started.'
                           .format(self.name, self.duration))
        # Foreleser publisher hvilken RAT som skal gjøres
        self.component.mqtt_client.publish(MQTT_TOPIC_OUTPUT, self.name)
        # Foreleser publisher hvor lang tid studentene har til rådighet (skal brukes i timer i UI)
        self.component.mqtt_client.publish(MQTT_TOPIC_OUTPUT, self.duration)
        

    def timer_completed(self):
        self._logger.debug('Timer for {} expired.'.format(self.name))
        # textfield som viser countdown sier nå:
        TimerExp = "Timer for RAT 01 exipred"
        # window['-TIMER-'].update(value=TimerExp)
        self.stm.terminate()
    ...
    * Send the messages listed below to the topic in variable `MQTT_TOPIC_INPUT`.
    ...
    def on_message(self, client, userdata, msg):
        ...
        * throw the message away.
        ...
        # Når et team klikker "Join RAT"
        if command == 'new_team_joined':
            try:
                print(type(self))
